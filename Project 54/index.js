...
const main = async () => {
  ...
  await client.set('favoriteflavor', 'ToDelete', {
    EXAT: parseInt(+new Date() / 1000) + 60 * 60 * 24,
  });
  console.log(await client.ttl('favoriteflavor')); // Expires: 86400
  // Set expire date = 12 hours from now
  await client.set('anotherflavor', 'ToDelete', {
    PXAT: parseInt(+new Date()) + 60 * 60 * 12 * 1000,
  });
  ...
  console.log(await client.getEx('limitedtimecoupon', { EX: 7200 })); // freeicecream
  console.log(await client.ttl('limitedtimecoupon')); // 7200
  // Remove key in 24 hours from now
  console.log(
    await client.getEx('limitedtimecoupon', {
      EXAT: parseInt(+new Date() / 1000) + 60 * 60 * 24,
    })
  ); // freeicecream
  ...
  console.log(
    JSON.stringify(await client.xRange('redemptions', '-', '+', { COUNT: 1 }))
  ); // []
  // Last record: [{"id":"1617314919000-0","message":{"couponcode":"weekendsale","userid":"5577"}}]
  console.log(
    JSON.stringify(
      await client.xRevRange('redemptions', '+', '-', { COUNT: 1 })
    )
  ); // []
  ...
  console.log(
    JSON.stringify(await client.hRandFieldCountWithValues('entrees', 2))
  );
  ...
  console.log(
    await client.smIsMember('winninglotto', ['1', '3', '7', '19', '22', '33'])
  );
